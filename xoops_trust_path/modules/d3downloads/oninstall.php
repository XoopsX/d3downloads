...
eval( ' function xoops_module_install_'.$mydirname.'( $module ) { return d3downloads_oninstall_base( $module , "'.$mydirname.'" ) ; } ' ) ;


if( ! function_exists( 'd3downloads_oninstall_base' ) ) {

function d3downloads_oninstall_base( $module , $mydirname )
{
	...
	if( defined( 'XOOPS_CUBE_LEGACY' ) ) {
		...
		$root->mDelegateManager->add( 'Legacy.Admin.Event.ModuleInstall.' . ucfirst($mydirname) . '.Success' , 'd3downloads_message_append_oninstall' ) ;
		$root->mDelegateManager->add( 'Legacy.Admin.Event.ModuleInstall.' . ucfirst($mydirname) . '.Fail' , 'd3downloads_message_append_oninstall' ) ;
		$ret = array() ;
	} else {
	...
	d3download_set_default_user_access( $mydirname ) ;
	return true ;
}

function d3downloads_message_append_oninstall( &$module_obj , &$log )
{
